...
void loop() {
  ...
  if (wifiConnected) {
    sendDataToServer(temperature, force, "temperature");
    sendDataToServer(force, temperature, "pressure");
  }

  delay(2000);
}

void sendDataToServer(float temperature, float force, const char* sensorType) {
  HTTPClient http;
  
  String url = "http://" + String(serverHost) + ":" + String(serverPort) + String(serverPath);
 
  String postData = "sensor_type=" + String(sensorType) + "&value=";

  if (strcmp(sensorType, "temperature") == 0) {
    postData += String(temperature, 2);
  } else if (strcmp(sensorType, "pressure") == 0) {
    postData += String(force, 2);  // Use 'force' for pressure
  }
